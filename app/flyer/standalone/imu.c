...
void imu_read(uint8_T dev_addr, uint8_T reg_offset, void *buf, int32_T n)
{
    /* 中断 读取 */
    if(HAL_OK != HAL_I2C_Mem_Read_IT(&s_imu_handle, dev_addr, reg_offset,
...
void imu_write(uint8_T dev_addr, uint8_T reg_offset, void *buf, int32_T n)
{ 
    if(HAL_OK != HAL_I2C_Mem_Write(&s_imu_handle, dev_addr, reg_offset,
