...
## 计划
1. 整理os.c stm32f4xx\_it.c中的内容 分别整入不同的文件中
2. 用系统调用实现osXXX函数(申明于cmsis\_os.h)
3. 实现Idle线程任务 该任务可以统计系统信息 检查任务堆栈 之后休眠处理器
4. 清理对HAL层的调用 直接调用寄存器级别 工程仅依赖于 CMSIS BSP目录
5. 不用标准库
6. 清理代码格式 不用tab等
7. 编写Cortex-M架构的基本文档(markdown格式)
...
[7]: http://write.blog.csdn.net/mdeditor
